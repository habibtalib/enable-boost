...
class ProcessBoostOpenCollection implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        /* This is for Payment Form (Open Collection) */
        $ii = $this->ii;
            $response = $billplz->toArray($billplz->getOpenCollectionIndex(array('page'=>strval($ii), 'status'=>'')));

        if (empty($response[1]['open_collections'])) {
            return;
        }

            $collection_id = [];

        foreach ($response[1]['open_collections'] as $col) {
